...
export function isLearnerProfile(value: unknown): value is LearnerProfile {
  ...
  return profile.version === 1 && typeof profile.id === 'string' && typeof profile.name === 'string' && typeof profile.totalAttempts === 'number' && typeof profile.nextSymbolIndex === 'number' && !!profile.skills && typeof profile.skills === 'object'
}

export function evaluateAttempt(metrics: AttemptMetrics) {
  const fastEnough = metrics.durationSeconds < 30
  const skillEnough = metrics.level === 1
    ? metrics.pathAccuracy >= masteryRules[1].accuracy
    : metrics.level === 2
      ? metrics.dotCoverage >= masteryRules[2].coverage
      : metrics.hintsUsed === 0
  return { strong: fastEnough && skillEnough, fastEnough, skillEnough }
}
...
export function chooseNextChallenge(profile: LearnerProfile): Challenge {
  ...
  if (!candidates.length) return { symbol: learningSymbols[0], level: 3 }
...
export function profileSummary(profile: LearnerProfile | null) {
  ...
  const proofs = skills.reduce((sum, skill) => sum + skill.proofs[1] + skill.proofs[2] + skill.proofs[3], 0)
  const totalProofs = learningSymbols.length * (masteryRules[1].required + masteryRules[2].required + masteryRules[3].required)
